...
*** install ycmd 
    ...
    - on ubuntu:
      - =sudo aptinstall build-essential cmake python-dev=
      - =sudo apt install ycmd=

*** configure ycmd for code completion
    #+begin_src emacs-lisp
      (use-package ycmd
	:ensure t
	:config
	(progn
	  ...
	  (set-variable 'ycmd-server-command '("python2" "/usr/bin/ycmd"))
	  (set-variable 'ycmd-global-config (expand-file-name "~/.ycmd/ycm_conf.py"))
	  (set-variable 'ycmd-extra-conf-whitelist (expand-file-name
						    ".ycm_conf.py" (projectile-project-info)))
	  (use-package company-ycmd
	    :ensure t
	    :init (company-ycmd-setup)
	    :config
	    (progn
	      (hook-company-backends 'c++-mode-hook '(company-ycmd))))))
    #+end_src
