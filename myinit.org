...
** Autopair
   For working with regular programming languages, such as Python and JS. See [[https://github.com/joaotavora/autopair][autopair]].
   #+BEGIN_SRC emacs-lisp
	 ...
	 ;; Autopair doesn’t make much sense when paredit-mode is turned on, so it actually defers to paredit-mode when that is installed and enabled. So disable autopair when paredit is turned on
